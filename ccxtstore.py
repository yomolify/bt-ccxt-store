...
class CCXTStore(with_metaclass(MetaSingleton, object)):
    ...
    def create_batch_order(self, orders):
        ...
        for order in orders:
            ...
            price = self.exchange.price_to_precision(order["symbol"], order["price"]),
            order['quantity'] = quantity[0]
            order['price'] = price[0]
            order['symbol'] = order['symbol'].replace('/', '')
            order['side'] = order["side"].upper()
            order['type'] = order["type"].upper()
        ...
        print(orders)
        orders = [self.exchange.encode_uri_component(self.exchange.json(order), safe=",") for order in orders]
        try:
            return [self.exchange.fapiPrivatePostBatchOrders({
